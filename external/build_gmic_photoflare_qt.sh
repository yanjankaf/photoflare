...
# they must be fetched/generated by the gmic Makefile before cmake runs.
# Without these, gmic-qt's CMakeLists version-check will fail.
echo "=== Fetching gmic generated headers (CImg.h, gmic_stdlib_community.h) ==="
for header in CImg.h gmic_stdlib_community.h; do
    if [[ ! -f "$GMIC_SRC/$header" ]]; then
        echo "  Generating $header ..."
        make -C "$GMIC_SRC" "$header"
    else
